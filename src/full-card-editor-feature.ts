...
function styleRows(orderedIds:string[]):void{
  const list=document.querySelector<HTMLElement>('.card-list');if(!list)return;const allowed=new Set(orderedIds);const rank=new Map(orderedIds.map((id,index)=>[id,index]));
  const rows=Array.from(list.querySelectorAll<HTMLElement>('[data-edit-card]'));
  rows.sort((a,b)=>(rank.get(a.dataset.editCard!)??999999)-(rank.get(b.dataset.editCard!)??999999));
  for(const row of rows){const id=row.dataset.editCard!;row.hidden=!allowed.has(id);row.classList.toggle('bulk-selected',selected.has(id));row.setAttribute('aria-pressed',bulkMode?String(selected.has(id)):'false');list.append(row);}
}
...
async function injectCatalogToolbar():Promise<void>{
  const list=document.querySelector<HTMLElement>('.card-list');if(!list||document.querySelector('[data-full-editor-toolbar]'))return;const {catalog}=await context();
  ...
  bar.querySelector<HTMLSelectElement>('[data-filter-status]')!.onchange=e=>{filters.status=(e.target as HTMLSelectElement).value as CardListFilters['status'];void applyListFilters();};
  bar.querySelector<HTMLSelectElement>('[data-filter-type]')!.onchange=e=>{filters.questionType=(e.target as HTMLSelectElement).value as QuestionType|'all';void applyListFilters();};
  bar.querySelector<HTMLSelectElement>('[data-filter-topic]')!.onchange=e=>{filters.topic=(e.target as HTMLSelectElement).value;void applyListFilters();};
  bar.querySelector<HTMLSelectElement>('[data-filter-sort]')!.onchange=e=>{filters.sort=(e.target as HTMLSelectElement).value as CardListFilters['sort'];void applyListFilters();};
  ...
  document.querySelector<HTMLInputElement>('#catalog-search')?.addEventListener('input',()=>queueMicrotask(()=>void applyListFilters()));
  await applyListFilters();
}

async function injectAdvancedEditor():Promise<void>{
  const form=document.querySelector<HTMLFormElement>('#card-form');if(!form||form.querySelector('[data-advanced-editor]'))return;const id=String(new FormData(form).get('id')??'');const {catalog}=await context();const card=catalog.cards.find(c=>c.id===id) as EditorCard|undefined;
  const block=document.createElement('section');block.dataset.advancedEditor='';block.className='advanced-editor';const sourceMeta=card?.sourceMeta;
  ...
  form.querySelector('.editor-actions')?.insertAdjacentElement('beforebegin',block);
  const danger=form.querySelector<HTMLElement>('[data-delete-card]');if(danger){danger.textContent='Archivieren';danger.title='Die Karte wird auf retired gesetzt und bleibt in Historie/Backups erhalten.';}
  enhancePreview(form,card);
}
...
function enhancePreview(form:HTMLFormElement,existing?:EditorCard):void{
  const aside=document.querySelector<HTMLElement>('.preview');if(!aside||aside.querySelector('[data-production-preview]'))return;const holder=document.createElement('div');holder.dataset.productionPreview='';holder.innerHTML='<span class="eyebrow">Produktionsvorschau</span><article class="question-card" data-editor-preview><h2></h2><div class="answer-box"><span class="eyebrow">Musterlösung</span><p></p></div></article>';aside.prepend(holder);
  const update=()=>{const card=previewCard(form,existing);if(!card)return;const q=holder.querySelector<HTMLElement>('.question-card')!;q.dataset.structuredCardId=card.id;q.querySelector('h2')!.textContent=card.prompt||'Fragevorschau';q.querySelector('.answer-box p')!.textContent=card.answer.modelAnswer||'Noch keine Musterlösung';};form.addEventListener('input',update);form.addEventListener('change',update);update();
}

async function saveEnhanced(form:HTMLFormElement):Promise<void>{
  const fd=new FormData(form);const id=String(fd.get('id')??'').trim();const {state,catalog}=await context();const existing=catalog.cards.find(c=>c.id===id) as EditorCard|undefined;if(existing?.status==='released')return;const candidate=updateCardFromForm(existing,fd);if(!candidate.id||!candidate.prompt||!candidate.answer.modelAnswer)throw new Error('ID, Frage und Musterlösung sind erforderlich.');const next=structuredClone(catalog);const index=next.cards.findIndex(c=>c.id===id);if(index>=0)next.cards[index]=candidate;else next.cards.push(candidate);syncKnowledgeItem(next,candidate);next.updatedAt=new Date().toISOString();state.catalogs=(state.catalogs??[]).map(c=>c.catalogId===catalog.catalogId?next:c);await saveState(state);location.reload();}
...
function captureClicks(event:Event):void{
  const target=event.target as Element|null;const row=target?.closest<HTMLElement>('[data-edit-card]');if(row&&bulkMode){event.preventDefault();event.stopImmediatePropagation();const id=row.dataset.editCard!;selected.has(id)?selected.delete(id):selected.add(id);void applyListFilters();return;}
  const form=document.querySelector<HTMLFormElement>('#card-form');if(!form)return;
  if(target?.closest('[data-save-card]')){const id=String(new FormData(form).get('id')??'');void context().then(({catalog})=>{const existing=catalog.cards.find(c=>c.id===id);if(existing?.status==='released')return;event.preventDefault();event.stopImmediatePropagation();void saveEnhanced(form).catch(error=>alert(error instanceof Error?error.message:String(error)));});}
  if(target?.closest('[data-delete-card]')){event.preventDefault();event.stopImmediatePropagation();if(confirm('Karte archivieren? Sie bleibt historisch erhalten.'))void archiveCurrent(form);}
  if(target?.closest('[data-json-card]')){event.preventDefault();event.stopImmediatePropagation();showJson(form);}
}
