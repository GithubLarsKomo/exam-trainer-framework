...
async function renderPanel():Promise<void>{
  const host=document.querySelector<HTMLElement>('.catalog-switch')?.parentElement;if(!host||host.querySelector('[data-catalog-lifecycle]'))return;
  const {catalogs,active}=await context();const archived=catalogs.filter(c=>c.archived);
